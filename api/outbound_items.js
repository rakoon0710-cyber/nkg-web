...
// ✅ 여러 후보 키 중 존재하는 컬럼을 찾아 값 리턴(정확 일치)
...
export default async function handler(req, res) {
  ...
  try {
    ...
    wmsRows.forEach((r) => {
      ...
      const mat = asText(r["상품코드"] || r["자재코드"]);
    ...
    const items = sapList.map((r) => {
      ...
      // ✅ sap자재자동 S열(작업여부: O/X) → work
      // 헤더가 '작업여부' / '작업 여부' / '작업' 등으로 변형되어도 잡힘
      const work =
        pickLoose(r, ["작업여부", "작업 여부", "작업", "WORK", "work"]) ||
        pick(r, ["작업여부", "작업 여부", "작업", "WORK", "work"]);
    ...
    // ─────────────────────────────────────────────
    // ✅ (필요 시 1회 디버그)
    // 아래 2줄을 잠깐 켜면, API 응답에 sap 첫행 컬럼명이 같이 내려와서
    // "작업여부" 헤더에 이상문자 붙었는지 바로 확인 가능
    //
    // const debugKeys = sapList[0] ? Object.keys(sapList[0]) : [];
    // return res.status(200).json({ ok: true, items, debugKeys });
    // ─────────────────────────────────────────────
